if (window.step == "2") {
    ...
    jQuery(document).ready(function () {
        ...
        if (width <= 768) {
            ...
            <div class="form_group_mobile">
                ...
                <div class="gender">
                   ...
                   <input type="hidden" class="relationship_hd" name="additionalinsured[${indexMobile}][Adult]" value="1" />
                    <select required name="additionalinsured[${indexMobile}][relationship]">
                        <option value=""></option>
                        <option disabled value="Self">${l10n.getText('Self')}</option>
            ...
            <div class="form_group_mobile">
                <div class="nationlity">
                    ...
                    <div class="__tootip"></div>
                </div>
            </div>
    
        </div>`));
    
            // Auto fill when checked mobile
            function ipKeyupM(inputValue, param) {
                let trInsurendMain = $(".table.adults #insurend_main_m")
                return trInsurendMain.find(param).val(inputValue);
            }
    
            $("#form_submit_detail_mobile .check").find("#checkbox").on('change', function () {
                // let x = number_form_adults;
                let trInsurendMain = $(".table.adults #insurend_main_m")
    
                if ($(this).is(':checked')) {
                    $("#msg_confirm").addClass('is-active');
                    trInsurendMain.find('.form_group_mobile').css('pointer-events', 'none');
    
                    trInsurendMain.find(`input[name='additionalinsured[${indexMobile}][insuredName]`).val($('.full_name').val());
                    trInsurendMain.find(`input[name='additionalinsured[${indexMobile}][dateOfBirth]`).val($('.date-picker').val());
                    // trInsurendMain.find('.custom-phone').text($('.date-picker').val());
                    trInsurendMain.find(`input[name='additionalinsured[${indexMobile}][insuredIDNumber]`).val($('.id_citizenship').val());
                    trInsurendMain.find(`select[name='additionalinsured[${indexMobile}][gender]`).val($('#gender_insurend').val());
                    trInsurendMain.find(`select[name='additionalinsured[${indexMobile}][nationality]`).val($('#insured_nationality').val());
                    trInsurendMain.find(`select[name='additionalinsured[${indexMobile}][relationship]`).val('Self');
    
                    $(".full_name").keyup(function () {
                        console.log('name')
                        ipKeyupM($(this).val(), `input[name='additionalinsured[${indexMobile}][insuredName]`);
                    })
    
                    $(".id_citizenship").keyup(function () {
                        console.log('citzen')
                        ipKeyupM($(this).val(), `input[name='additionalinsured[${indexMobile}][insuredIDNumber]`);
                    })
    
                    $("#gender_insurend").change(function () {
                        console.log('gender')
                        ipKeyupM($(this).val(), `select[name='additionalinsured[${indexMobile}][gender]`);
                    });
    
                    $("#insured_nationality").change(function () {
                        console.log('nationality')
                        ipKeyupM($(this).val(), `select[name='additionalinsured[${indexMobile}][nationality]`);
                    });
                    ...
                    trInsurendMain.find('td').css('pointer-events', 'auto');
                    trInsurendMain.find(`select[name='additionalinsured[${indexMobile}][relationship]`).val('');
                    trInsurendMain.find(".custom-phone").text('')
                    trInsurendMain.find(`input[name='additionalinsured[${indexMobile}][dateOfBirth]`).val('');
    
                    $(`input[name='additionalinsured[${indexMobile}][insuredName]`).val("");
                    $(`input[name='additionalinsured[${indexMobile}][insuredIDNumber]`).val("");
                    $(`select[name='additionalinsured[${indexMobile}][gender]`).val("");
                    $(`select[name='additionalinsured[${indexMobile}][nationality]`).val("");
    
                    inputUnbind($("#full_name"), 'keyup')
                    inputUnbind($(".id_citizenship"), 'keyup')
                    inputUnbind($("##gender_insurend"), 'change')
                    inputUnbind($("#insured_nationality"), 'change')
            ...
            if ($("#form_submit_detail .children").find('tr').length > 0) {
                index_total -= 2;
            } else {
                index_total -= 1;
            }
            console.log(index_total)
            ...
            <td class="date-calendear-adults">
               <label for="" class="custom-phone"></label>
                <input autocomplete="off" required type="text" name="additionalinsured[${index_total}][dateOfBirth]" id="datefilter_adults[${index_total}]" class="datefilter_adults" data-type="date-picker" onkeypress='return event.charCode >1000 && event.charCode <= -10000' class="phone_number">
                <i class="fa fa-calendar" aria-hidden="true"></i>
            ...
            <td>
               <input type="hidden" class="relationship_hd" name="additionalinsured[${index_total}][Adult]" value="1" />
                <select required name="additionalinsured[${index_total}][relationship]">
                    <option value=""></option>
                    <option hidden value="Self">${l10n.getText('Self')}</option>
                    <option value="Husband">${l10n.getText('Husband')}</option>
                    <option value="Wife">${l10n.getText('Wife')}</option>
                    <option value="Father">${l10n.getText('Father')}</option>
                    <option value="Mother">${l10n.getText('Mother')}</option>
            ...
            $("#form_submit_detail .check").find("#checkbox").on('change', function () {
                // let x = number_tr_adults;
                let trInsurendMain = $(".table.adults #insurend_main")
                if ($(this).is(':checked')) {
    
                    $("#msg_confirm").addClass('is-active');
                    // trInsurendMain.find('td select').prop("disabled", true);
                    trInsurendMain.find('td').not(':last-child').css("pointer-events", "none");
                    trInsurendMain.find('td:last-child select').css('pointer-events', 'none');
    
                    // $(".tooltip_autofill").css('pointer-events', 'auto');
                    trInsurendMain.find(`input[name='additionalinsured[${index_total}][insuredName]`).val($('#full_name').val());
                    trInsurendMain.find(`input[name='additionalinsured[${index_total}][dateOfBirth]`).val($('.date-picker').val());
                    trInsurendMain.find('.custom-phone').text($('.date-picker').val());
                    trInsurendMain.find(`input[name='additionalinsured[${index_total}][insuredIDNumber]`).val($('.id_citizenship').val());
                    trInsurendMain.find(`select[name='additionalinsured[${index_total}][gender]`).val($('.gender').val());
                    trInsurendMain.find(`select[name='additionalinsured[${index_total}][nationality]`).val($('#insurend_nationality').val());
                    trInsurendMain.find(`select[name='additionalinsured[${index_total}][relationship]`).val('Self');
                    // trInsurendMain.find(`select[name='additionalinsured[${index_total}][relationship]`).prop('disabled', false);
                    ...
                    $("#full_name").keyup(function () {
                        console.log("name")
                        inputKeyUp($(this).val(), `input[name='additionalinsured[${index_total}][insuredName]`);
                    })
    
                    $(".id_citizenship").keyup(function () {
                        console.log("citizenship")
                        inputKeyUp($(this).val(), `input[name='additionalinsured[${index_total}][insuredIDNumber]`);
                    })
    
                    $(".gender").change(function () {
                        console.log("gender")
                        inputKeyUp($(this).val(), `select[name='additionalinsured[${index_total}][gender]`);
                    });
    
                    $("#insurend_nationality").change(function () {
                        console.log("nationality")
                        inputKeyUp($(this).val(), `select[name='additionalinsured[${index_total}][nationality]`);
                    });
                    ...
                    $("#msg_confirm").removeClass('is-active');
                    $(".tooltip_autofill").css('pointer-events', 'none');
                    trInsurendMain.find('td').not('.tooltip_autofill').css('pointer-events', 'auto');
                    trInsurendMain.find('td:last-child select').css('pointer-events', 'auto');
                    trInsurendMain.find(`select[name='additionalinsured[${index_total}][relationship]`).val('');
                    trInsurendMain.find(".custom-phone").text('')
                    trInsurendMain.find(`input[name='additionalinsured[${index_total}][dateOfBirth]`).val('');
    
                    $(`input[name='additionalinsured[${index_total}][insuredName]`).val("");
                    $(`input[name='additionalinsured[${index_total}][insuredIDNumber]`).val("");
                    $(`select[name='additionalinsured[${index_total}][gender]`).val("");
                    $(`select[name='additionalinsured[${index_total}][nationality]`).val("");
    
                    inputUnbind($("#full_name"), 'keyup')
                    inputUnbind($(".id_citizenship"), 'keyup')
                    inputUnbind($(".gender"), 'change')
                    inputUnbind($("#insurend_nationality"), 'change')
        ...
        // Hover show text of field nationality
        $(".nationality").each(function (index) {
            $(this).parent().hover(function () {
                // console.log($(this).children("option:selected").text());
                console.log('hover');
                let optionSelected = $(this).children().children("option:selected").text();
                console.log('optionSelected', optionSelected);
                if (optionSelected != '') {
                    $(this).children().siblings('.__tootip').html(optionSelected);
                    $(this).children().siblings('.__tootip').addClass('is-hover');
                ...
                $(this).children().siblings('.__tootip').removeClass('is-hover');
            });
        });
        ...
        $('input[class="date-picker"]').on('apply.daterangepicker', function (ev, picker) {
            ...
            const checkInsurend = $("#form_submit_detail .check").find("#checkbox")
            const checkInsurendMobile = $("#form_submit_detail_mobile .check").find("#checkbox")
    
    
            if (checkInsurend.is(":checked")) {
                $(".table.adults #insurend_main").find(".custom-phone").text($(this).val())
                $(".table.adults #insurend_main").find(`input[name='additionalinsured[${index_total}][dateOfBirth]`).val($(this).val());
            }
    
            if (checkInsurendMobile.is(":checked")) {
                $(".table.adults #insurend_main_m").find(`input[name='additionalinsured[${number_form_adults}][dateOfBirth]`).val($(this).val());
            }
    
    
            if (checkInsurend.is(":checked") && birthDay > 75) {
                checkInsurend.prop("checked", false);
                $("#insurend_main").find('td').css('pointer-events', 'auto');
                $("#insurend_main").find('td select').css('pointer-events', 'auto');
                $("#insurend_main").find('input').val('');
                $("#insurend_main").find('select').val('');
                $("#insurend_main").find('.custom-phone').text('');
                $("#msg_confirm").removeClass('is-active');
            }
    
            if (checkInsurendMobile.is(":checked") && birthDay > 75) {
                checkInsurendMobile.prop("checked", false);
                $("#insurend_main_m").find('.form_group_mobile').css('pointer-events', 'auto');
    
                $("#insurend_main_m").find('input').val('');
                $("#insurend_main_m").find('select').val('');
                $("#msg_confirm").removeClass('is-active');
            }
        ...
        $('.date-calendear-adults').daterangepicker({
            ...
            minYear: 1910,
            autoApply: true,
            startDate: moment(new Date()).subtract('years', 18),
            maxDate: moment(new Date()).subtract('years', 18),
            locale: {
                format: 'YYYY/MM/DD'
            }
        });
        ...
        $('.date-calendear-children').daterangepicker({
            ...
            showDropdowns: true,
            minYear: 1901,
            minDate: moment(new Date()).subtract('years', 23),
            autoApply: true,
        ...
        keyUpIDNumber = (El) => {
            $(El).keyup(function () {
                ...
                if (citizenshipIDValue && !checkInsurend.is(":checked")) {
        ...
        keyUpIDNumberMobile = (El) => {
            $(El).keyup(function () {
                let valueArr = [];
                const checkInsurend = $("#form_submit_detail_mobile .check").find("#checkbox")
                const citizenshipIDValue = $('.id_citizenship').val();
                if (citizenshipIDValue && !checkInsurend.is(":checked")) {
                    valueArr.push(citizenshipIDValue);
                }
        ...
        // Fech data JSON Country
         fetch('../../assets/files/data/countries.' + AppConfigs.UniqueSeoCode + '.json?t=100001')
            ...
            .then(function (data) {
                ...
                $("#insurend_nationality").html(listItems.join(''))
    
                $(".mobile_form").find("select[class='nationality']").each(function () {
                    document.getElementById(`${$(this).attr("id")}`).innerHTML = listItems.join('');
                });
